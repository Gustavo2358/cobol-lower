...
## Preserve physical transfer obligations

Logical FULL_IDENTITY MOVE is used only when the destination has no physical
view in the regional translation. A destination with a physical view keeps the
existing regional transfer and codec proof path. A logical identity fact does
not discharge byte encoding preconditions. The existing CICS physical-profile
consumer test caught this distinction and remains part of FAST.
