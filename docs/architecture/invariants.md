...
Cada definição tem ID estável, regra e evals. **Enforcement por escopo implementado em WORK-LOWER-001, detalhado no JSON.** SELF_REVIEW_WITH_AUTOMATED_CHECKS distingue review técnico de oracles automatizados; não afirma review independente/humano. Não anunciar AUTOMATED antes de existir executor e contracaso que demonstre a proteção. O JSON adjacente indexa as definições; o texto da regra não ganha significado novo por ser serializado.
...
Enforcement: `SELF_REVIEW_WITH_AUTOMATED_CHECKS` no escopo registrado no JSON. Sem exceção silenciosa; limites legítimos devem ser explicitados no perfil/work item.
...
Enforcement: `AUTOMATED_VERIFIED` no escopo registrado no JSON. Sem exceção silenciosa; limites legítimos devem ser explicitados no perfil/work item.
