...
## Remediação do review — mesmo PR #1

Base do delta: `149acd2acfcedbd3f847cccd945997af59d04a4e`. Autorização: corrigir os cinco findings do review humano desta preparação, mantendo branch/PR e sem iniciar implementação. O commit de remediação preserva o anterior; não há amend/squash. O resultado da primeira self-review acima é histórico, não encerra os findings recebidos depois.

| Finding | Correção e contraprova |
| --- | --- |
| P1 — granted vazio/null e IDs malformados | Schema exige lista não vazia e current string para todo granted; IDs de autorização/current/dependências seguem CP + dígitos. Seis negativos foram reproduzidos antes (exit 1 por aceitação indevida) e rejeitados depois. |
| P1 — push sem CI verde | COMMIT/PUSH e ADVANCE exigem terminal PASS de cada check obrigatório no SHA publicado; certificado local não substitui recovery anterior até essa prova. FREEZE fixa conjunto e limite de espera; recibo no PR/handoff evita commit recursivo de resultado do próprio CI. CP4 não pode iniciar CP5 com CI pendente/falho/ausente. |
| P2 — tentativas sem progresso | Regra de hipótese nova restaurada; mesma falha sem evidência nova e alternância de reparos equivalentes exigem blocker factual e parada humana. Retomada preserva tentativas; limite de espera remoto não reinicia por rerun. |
| P2 — identidade duplicada | Um candidate_diff_sha256 canônico; review aponta para ele. reviewed_head é somente HEAD pré-commit; review.reviewed_commit removido. Troca de candidato invalida review; G-DOCS deverá rejeitar duplicatas legadas. |
| P3 — digest FREEZE indefinido | Lista path + SHA-256 dos bytes exatos + revisão Git por arquivo, incluindo source lock; sem digest agregado/normalização. Hash recomputado via Git e sha256sum; mutação de um newline detectada e bytes originais novamente verdes. |

Comandos desta remediação: `python3 /tmp/check-review-schema.py` (RED exit 1 antes; GREEN exit 0 depois, três positivos/seis negativos e segundo GREEN); `python3 /tmp/check-review-evidence.py` (exit 0); `python3 /tmp/check-long-running.py` (exit 0, regressão dos sete contracasos e integridade documental); `python3 /tmp/check-recovery.py` (exit 0); `git diff --check` (exit 0). O auditor original referencia agora explicitamente a baseline a429abd para seu contracaso histórico. Mutações só em memória/repo descartável; nenhum resíduo no checkout.

Limites do schema confirmados deliberadamente: CP999 tem formato válido; sua inexistência no plano, current CP1 fora de [CP0] e dependências sem certificado são checks relacionais de G-DOCS especificados, ainda não instalados. Não alegar rejeição desses casos pelo JSON Schema. Nenhum teste remoto ou engine anti-spin foi implementado nesta remediação.

Challenge documental adicional (self-review): CI local verde + remoto pending/missing/fail não avança; success em SHA antigo não vale; todos os checks obrigatórios precisam estar presentes e em success no SHA correto; skipped/neutral/cancelled não contam; timeout preserva recovery anterior. Corrigir CI permanece no CP atual, gera novo SHA e nova validação, sem rewrite do anterior. São regras revisadas, não testes executados de G-GIT. Consulta real `gh pr view 1 --json statusCheckRollup,headRefOid,body` retornou exit 0 e lista vazia no head de preparação; não é PASS de CI. Este escopo docs-only continua sem workflow ou obrigação de CI executável, conforme gates catalogados não implementados.

Diff e consistência manifesto/state/proposals/active/history/registry/backlog/índice revisados novamente. Estado permanece proposta não autorizada, CP0..CP5 not_started; contratos e source lock intactos. Próximo passo não iniciado: novo review humano do PR, sem merge/auto-merge. Executores e prova remota reais continuam dependentes de CP0 futuramente autorizado.
