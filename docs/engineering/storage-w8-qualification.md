...
Only published SP facts are translated. One physical allocation per component, retained unknown declaration objects and explicit initial uncertainty; unknown Region bounds never become total initial slices. No source matching, PICTURE parsing or first-invocation inference.

## Validation

Full semantic reactor with performance enabled, architecture, FastSuite/FastAdapterSuite and 21 harness tests passed. Entry-bounds fix: reduced RED/GREEN plus complete FastAdapterSuite passed.
