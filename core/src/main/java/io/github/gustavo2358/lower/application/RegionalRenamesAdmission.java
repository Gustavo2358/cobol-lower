...
import io.github.gustavo2358.lower.domain.SpInput.UnitKey;

/** Closure of source-proved aliases. Does not resolve or interpret COBOL names. */
final class RegionalRenamesAdmission {
    private RegionalRenamesAdmission() { }
    static void validate(UnitKey unit,Inventory inventory,Map<NodeId,Node> nodes,
            Map<NodeId,View> views,Set<RelationId> relationIds) {
        ...
        var roots=new HashMap<NodeId,NodeId>();
        for(var node:nodes.keySet()) {
            var path=new ArrayList<NodeId>();var cursor=node;
        ...
        for(var r:inventory.renames()) {
            require(r.status()==RelationStatus.PROVEN?r.from().isPresent()&&r.gapCodes().isEmpty():!r.gapCodes().isEmpty(),"RENAMES proof or explicit gap required");
            var owner=nodes.get(r.owner());var view=views.get(r.owner());
            if(r.status()==RelationStatus.UNPROVEN) {
            ...
            if(r.through().isPresent()) {
                require(!first.id().equals(last.id())&&owner.kind()==Kind.GROUP,"THROUGH requires distinct endpoints and group interpretation");
    ...
    private static void require(boolean condition,String reason) {if(!condition)throw new RegionalStorageAdmission.Invalid(reason);}
}
