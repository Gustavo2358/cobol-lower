...
/** CP6 PERFORM BASIC publication assembly; no transport dependencies or universal statement framework. */
final class PerformLowerer implements LowerInput {
    @Override public LoweringResult lower(SpInput input, Options options) {
        var plan = new PerformAdmission().plan(input, options.admission()); var admission = plan.admission();
        ...
        var revision = CanonicalRevision.perform(input, plan, options.maximumIdentityCharacters());
        ...
        var assembly = PerformSequenceAssembler.assemble(plan, data, unit, entryOrigin, ids, origins, statements, operands, items, uncertainties);
        ...
        var unitOrigin = origins.derived("unit-origin", List.of(entryOrigin, assembly.entrySequenceOrigin()), "perform-basic@1/selected-unit");
