...
public final class CobolLowerer implements LowerInput {
    @Override public LoweringResult lower(SpInput input, Options options) {
        if (input != null && input.statements().stream().anyMatch(SpInput.PerformFact.class::isInstance))
            return new PerformLowerer().lower(input, options);
        if (input != null && input.statements().stream().anyMatch(SpInput.IfFact.class::isInstance))
            return new IfLowerer().lower(input, options);
        if (input != null && input.statements().stream().anyMatch(SpInput.CallFact.class::isInstance))
            return new CallLowerer().lower(input, options);
