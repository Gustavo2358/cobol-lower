...
final class FileAuxiliaryLowering {
    ...
    LabelId prefix(FileFacts.Use use,LabelId entry,OriginId origin,LocalIds local,List<Sequence> out){
        ...
        var current=entry;for(var c:selected){var key=use.statement().handle()+"/"+use.ordinal()+"/"+c.id();
            ...
            var invoke=new Operations.Invoke(header(op,derived,contract),"checkpoint",target,List.of(),List.of(),new Interactions.ExternalSignature(signature),List.of(),new Interactions.EffectBound(new Interactions.ForeignEffects(new Scopes.WithinMemory(new Scopes.VisibleMemory(unit,true)),Scopes.NoMemory.INSTANCE,List.of()),List.of()),new Control.InvocationOutcomes(List.of(new Control.Normal(current)),new Scopes.WithinControl(new Scopes.UnitControl(unit,false,true,true,false,false,false))),new Interactions.UnknownContract(contract));
