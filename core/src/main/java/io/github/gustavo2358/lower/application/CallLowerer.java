...
/** CP6 CALL publication assembly; no transport dependencies or universal statement framework. */
final class CallLowerer implements LowerInput {
    @Override public LoweringResult lower(SpInput input, Options options) {
        var plan = new CallAdmission().plan(input, options.admission()); var admission = plan.admission();
        ...
        var revision = CanonicalRevision.call(input, plan, options.maximumIdentityCharacters());
