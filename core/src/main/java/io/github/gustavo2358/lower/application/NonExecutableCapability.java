...
public record NonExecutableCapability(SpInput.StatementId statement, Kind kind,
    ...
    static Optional<NonExecutableCapability> of(SpInput.StatementFact fact) {
        Kind kind = switch (fact) {
            case SpInput.CicsHandlerFact ignored -> Kind.CICS_HANDLER;
            case SpInput.CicsAbendFact ignored -> Kind.CICS_ABEND;
            case SpInput.CicsCommandFact ignored -> Kind.CICS_COMMAND;
            default -> null;
        };
