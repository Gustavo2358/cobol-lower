...
final class OpaqueOperands {
    ...
    static Known translate(SpInput.StatementFact fact,OperationId operation,ScalarDataTranslator.Result data,
        ...
        if(fact instanceof SpInput.ProcedurePerformFact p){p.loop().ifPresent(l->refs.addAll(l.conditionReads()));p.times().flatMap(SpInput.PerformCount::reference).ifPresent(refs::add); }
