...
public final class IfAdmission implements AdmitInput {
    ...
    static void admitPredicate(StatementHeader header,String shape,Provenance origin,PredicateGuarantee predicate,
            ...
            need(c,predicate.knownReads().size()==1 && predicate.knownReads().equals(reads.stream().map(DataReference::id).toList()),"all known read occurrences in published order");
            for(var read:reads) {
                c.touch();CallAdmission.admitReference(read,OperandRole.READ,header,c);
