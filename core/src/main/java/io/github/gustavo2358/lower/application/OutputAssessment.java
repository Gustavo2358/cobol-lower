...
public record OutputAssessment(LoweringResult.Status status, Optional<Publication> publication, ValidationResult validation) {
    public static OutputAssessment assess(Publication publication, ValidationOptions options) {
        ...
        var status = switch (result.status()) {
            ...
            case INCOMPLETE_VALIDATION -> LoweringResult.Status.VALIDATION_INCOMPLETE;
            case STRUCTURALLY_VALID -> LoweringResult.Status.SUCCESS;
        };
        return new OutputAssessment(status, status == LoweringResult.Status.SUCCESS ? Optional.of(publication) : Optional.empty(), result);
    }
}
