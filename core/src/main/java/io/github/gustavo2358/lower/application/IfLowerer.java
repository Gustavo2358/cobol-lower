...
/** SP1.4 diamond publication; entry authority is explicit and premises remain upstream claims. */
final class IfLowerer implements LowerInput {
    @Override public LoweringResult lower(SpInput input, Options options) {
        var plan = new IfAdmission().plan(input, options.admission()); var admission = plan.admission();
        ...
        var revision = CanonicalRevision.simpleIf(input, plan, options.maximumIdentityCharacters());
        ...
        var assembly = IfSequenceAssembler.assemble(plan, data, unit, entryOrigin, ids, origins, statements, operands, items, uncertainties);
        ...
        var unitOrigin = origins.derived("unit-origin", List.of(entryOrigin, assembly.entrySequenceOrigin()), "simple-if@1/selected-unit");
