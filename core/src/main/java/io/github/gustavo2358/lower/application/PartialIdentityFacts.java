...
final class PartialIdentityFacts {
    ...
    static void write(Object fact,Consumer<String> field,Consumer<Object> value) {
        switch(fact) {
            ...
            case FileFacts.Use r -> {field.accept("FileUse");value.accept(r.statement());value.accept(r.ordinal());value.accept(r.command());value.accept(r.mode());value.accept(r.profile());value.accept(r.bindingStatus());value.accept(r.candidates());value.accept(r.provenance());value.accept(r.gapCodes());if(r.surface().isPresent()){field.accept("FileSurface");value.accept(r.surface().orElseThrow());}if(r.effects().isPresent()){field.accept("FileEffects");value.accept(r.effects().orElseThrow());}}
