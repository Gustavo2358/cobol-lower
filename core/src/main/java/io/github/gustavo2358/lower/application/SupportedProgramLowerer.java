...
final class SupportedProgramLowerer implements LowerInput {
    @Override public LoweringResult lower(SpInput input, Options options) {
        var plan = new SupportedProgramAdmission().plan(input, options.admission()); var admission = plan.admission();
        ...
        var revision = CanonicalRevision.supportedProgram(input, plan, options.maximumIdentityCharacters());
        ...
        var assembly = SupportedProgramAssembler.assemble(plan, data, unit, entryOrigin, ids, origins, statements, operands, items, uncertainties);
